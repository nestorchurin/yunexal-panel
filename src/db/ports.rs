...
/// Returns a map for all allocated ports of a server: (host_port, container_port) → (tag, enabled, ufw_blocked).
pub async fn get_port_tags(
    pool: &Pool<Sqlite>,
    server_id: i64,
) -> Result<HashMap<(i64, i64), (String, bool, bool)>> {
    let rows: Vec<(i64, i64, String, i64, i64)> = sqlx::query_as(
        "SELECT host_port, container_port, tag, enabled, COALESCE(ufw_blocked,0) FROM server_ports WHERE server_id = ?",
    )
    .bind(server_id)
    .fetch_all(pool)
    .await
    .context("get_port_tags")?;
    Ok(rows.into_iter().map(|(hp, cp, tag, en, ub)| ((hp, cp), (tag, en != 0, ub != 0))).collect())
}
...
/// Sets the ufw_blocked flag for a port entry.
pub async fn set_port_ufw_blocked(
    pool: &Pool<Sqlite>,
    server_id: i64,
    host_port: i64,
    container_port: i64,
    ...
    sqlx::query(
        r#"INSERT INTO server_ports (server_id, host_port, container_port, tag, ufw_blocked)
           VALUES (?, ?, ?, '', ?)
           ON CONFLICT(server_id, host_port, container_port) DO UPDATE SET ufw_blocked = excluded.ufw_blocked"#,
    )
    .bind(server_id)
    .bind(host_port)
    .bind(container_port)
